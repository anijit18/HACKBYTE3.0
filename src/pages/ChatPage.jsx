...
// ✅ Initialize default "Hello World" messages for all users
const initializeMessages = () => {
  const defaultMessages = {};
  users.forEach(user => {
    defaultMessages[user.id] = [{ sender: user.name, text: 'Hello User👋🏻' }];
  });
...
const ChatPage = () => {
  ...
  const [chatMessages, setChatMessages] = useState(initializeMessages);
  ...
  return (
    <div className="flex h-screen bg-gradient-to-r from-blue-100 via-purple-100 to-pink-100 dark:from-gray-900 dark:via-gray-800 dark:to-gray-900 text-gray-900 dark:text-gray-100 mt-20 transition-all duration-500">
      ...
      {/* Sidebar */}
      <aside className="hidden md:flex flex-col w-64 bg-white dark:bg-gray-800 shadow-2xl rounded-tr-3xl rounded-br-3xl p-5 transition-all duration-500 animate-sidebar-slide-in">
        ...
        <div className="space-y-2 overflow-y-auto scrollbar-thin scrollbar-thumb-blue-300 dark:scrollbar-thumb-blue-600">
          {users.map((u) => (
            <div
              ...
              className={`flex items-center gap-3 cursor-pointer transition-all duration-300 transform hover:scale-[1.02] p-3 rounded-xl font-medium ${
                selectedUserId === u.id
      ...
      <main className="flex flex-col flex-1">
        ...
        {/* Header */}
        <header className="flex items-center justify-between px-4 py-4 bg-white dark:bg-gray-800 shadow-md border-b dark:border-gray-700 transition duration-300">
        ...
        {/* Chat messages */}
        <div className="flex-1 overflow-y-auto p-6 space-y-4 scrollbar-thin scrollbar-thumb-blue-200 dark:scrollbar-thumb-blue-700 transition-all duration-500">
          {messages.map((msg, i) => (
            <div
              key={i}
        ...
        {/* Input area */}
        <div className="pb-30 px-6 pb-5 bg-white dark:bg-gray-800 border-t border-gray-200 dark:border-gray-700 transition-all duration-300">
          <div className="flex items-center gap-3">
            <input
              type="text"
            ...
            <button
              onClick={handleSend}
              className="bg-gradient-to-r from-blue-500 to-indigo-500 hover:from-indigo-600 hover:to-purple-600 text-white p-2 rounded-full shadow-lg transition-all duration-300"
            >
